# -*- coding: utf-8 -*-
"""Overlap Similarity.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1j8wil3BgtoREplMpis-5c-Gtwvs5r5mm
"""
...
def overlap_similarity(set1, set2):
    ...
    if len(set1) < len(set2):
        min_cardinality_set = set1
        min_cardinality = len(set1)
    elif len(set2) < len(set1):
        min_cardinality_set = set2
        min_cardinality = len(set2)
    else:
        min_cardinality_set = None
        min_cardinality = len(set1)  # or len(set2), since they have the same cardinality
    return (intersection) / min_cardinality
...
for i, disease1 in enumerate(disease_names):
    ...
    for j, disease2 in enumerate(disease_names):
        if i != j:  # Avoid comparing a disease to itself
            ...
            similarity = overlap_similarity(set1, set2)
            similarity_matrix[i, j] = similarity


# Display the Sorensen similarity matrix
overlap_df = pd.DataFrame(similarity_matrix, index=disease_names, columns=disease_names)
print("Overlap Similarity Matrix:")
print(overlap_df)

# Export the Jaccard similarity matrix to a CSV file
overlap_df.to_csv('overlap_similarity_new_latest.csv')
...
import pandas as pd
import io
predicted_df = pd.read_excel(io.BytesIO(uploaded['OS.xlsx'])).set_index('DOID')
print(predicted_df)
...
import pandas as pd
import io
similarity_matrix = pd.read_excel(io.BytesIO(uploaded['OS (1).xlsx']))
...
thresholded_df.to_excel('thresholded_values66.xlsx')
...
def build_roc_curve(true_labels, predicted_values):
    ...
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive Rate')
    plt.title('ROC Curve For Overlap Similarity')
    plt.legend(loc='lower right')
    plt.grid(True)
    plt.show()
...
file_path_predicted_values = '/content/thresholded_values66.xlsx'
...
"""Normalizing Calculated Similarity Values Using Normalization Technique"""
...
import pandas as pd
import io
similarity_matrix_overlap = pd.read_excel(io.BytesIO(uploaded['OS.xlsx'])).set_index('DOID')
print(similarity_matrix_overlap)
...
normalized_similarity_matrix = min_max_normalize(similarity_matrix_overlap)

# Create a DataFrame for the normalized matrix
normalized_df = pd.DataFrame(normalized_similarity_matrix, columns=similarity_matrix_overlap.columns, index=similarity_matrix_overlap.index)

# Export the normalized matrix to an Excel file
normalized_df.to_excel("normalized_matrix_overlap.xlsx")
...
import pandas as pd
import io
predicted_df= pd.read_excel(io.BytesIO(uploaded['normalized_matrix_overlap (1).xlsx'])).set_index('DOID')
print(predicted_df)
...
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('Overlap Receiver Operating Characteristic (ROC) Curve')
plt.legend(loc='lower right')
plt.show()
